...
if (!empty($_POST['carte']) && !empty($_POST['question']) && !empty($_POST['reponse']) && !empty($_POST['nouvellequestion']) && !empty($_POST['nouvellereponse'])) {
	require 'db.php';
	$carte = $_POST['carte'];
	$question = $_POST['question'];
	$reponse = strtolower($_POST['reponse']);
	$stmt = $db->prepare('SELECT question, reponse FROM digimindmap_cartes WHERE url = :url');
	if ($stmt->execute(array('url' => $carte))) {
		...
		if ($question === $questionSecrete && password_verify($reponse, $reponseSecrete)) {
			...
			$stmt = $db->prepare('UPDATE digimindmap_cartes SET question = :nouvellequestion, reponse = :nouvellereponse WHERE url = :url');
			if ($stmt->execute(array('nouvellequestion' => $nouvellequestion, 'nouvellereponse' => $nouvellereponse, 'url' => $carte))) {
				$_SESSION['digimindmap'][$carte]['reponse'] = $nouvellereponse;
				echo 'acces_modifie';
			} else {
				echo 'erreur';
			}
