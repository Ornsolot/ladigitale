...
if (!empty($_POST['nuage']) && !empty($_POST['question']) && !empty($_POST['reponse']) && !empty($_POST['nouvellequestion']) && !empty($_POST['nouvellereponse'])) {
	require 'db.php';
	$nuage = $_POST['nuage'];
	...
	if ($stmt->execute(array('url' => $nuage))) {
		...
		if ($question === $questionSecrete && password_verify($reponse, $reponseSecrete)) {
			...
			$stmt = $db->prepare('UPDATE digiwords_nuages SET question = :nouvellequestion, reponse = :nouvellereponse WHERE url = :url');
			if ($stmt->execute(array('nouvellequestion' => $nouvellequestion, 'nouvellereponse' => $nouvellereponse, 'url' => $nuage))) {
				$_SESSION['digiwords'][$nuage]['reponse'] = $nouvellereponse;
				echo 'acces_modifie';
			} else {
				echo 'erreur';
			}
